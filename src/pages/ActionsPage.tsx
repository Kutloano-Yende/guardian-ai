...
import { Plus, ListTodo, Clock, CheckCircle2, AlertOctagon, Timer, MoreHorizontal, Pencil, Archive, Trash2 } from "lucide-react";
...
export default function ActionsPage() {
  ...
  const openEdit = (a: ActionItem) => { setForm({ name: a.name, relatedType: a.relatedType, relatedId: a.relatedId, assignedTo: a.assignedTo, priority: a.priority, startDate: a.startDate || "", dueDate: a.dueDate, status: a.status, notes: a.notes, estimatedImpactOfDelay: a.estimatedImpactOfDelay }); setEditingId(a.id); setOpen(true); };
  const handleArchive = (a: ActionItem) => { updateItem("actions", a.id, { status: "closed" as any }); };
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); if (editingId) { updateItem("actions", editingId, form); } else { addAction(form); } setForm(DEFAULT_FORM); setEditingId(null); setOpen(false); };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-display font-bold text-foreground">Action Plans</h1><p className="text-muted-foreground mt-1">Track mitigation actions and task follow-ups</p></div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><Label className="text-white">Start Date</Label><Input type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} /></div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(v) => { if (!v) setDeleteTarget(null); }}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Action?</AlertDialogTitle><AlertDialogDescription>This will permanently delete "{deleteTarget?.name}". This action cannot be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => { if (deleteTarget) { deleteItem("actions", deleteTarget.id); setDeleteTarget(null); } }} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
      </AlertDialog>
      ...
      {actions.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card p-5">
            ...
            <div className="relative h-48"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={priorityData} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="value" animationDuration={800}>{priorityData.map((e, i) => <Cell key={i} fill={e.color} stroke="transparent" />)}</Pie><Tooltip content={<GlassTooltip />} /></PieChart></ResponsiveContainer><div className="absolute inset-0 flex items-center justify-center pointer-events-none"><span className="text-2xl font-bold text-foreground">{actions.length}</span></div></div>
            <div className="flex justify-center gap-3 mt-2">{priorityData.map((p) => (<div key={p.name} className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full" style={{ background: p.color }} /><span className="text-[11px] text-muted-foreground">{p.name}</span></div>))}</div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="glass-card p-5">
            ...
            {overdue.length === 0 ? (<p className="text-muted-foreground text-sm py-8 text-center">No overdue actions!</p>) : (
              <div className="space-y-3 max-h-56 overflow-y-auto">{overdue.map((a) => (<div key={a.id} className="flex items-center justify-between p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.1)" }}><div><p className="text-sm font-medium text-foreground">{a.name}</p><p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5"><Clock className="w-3 h-3" /> Due: {new Date(a.dueDate).toLocaleDateString()}</p></div><SeverityBadge severity={a.priority} /></div>))}</div>
            )}
          </motion.div>
        </div>
      )}
      ...
      {actions.length === 0 ? (
        <div className="glass-card p-12 text-center"><ListTodo className="w-12 h-12 mx-auto text-muted-foreground mb-4" /><h3 className="font-display font-semibold text-foreground">No action items</h3><p className="text-muted-foreground text-sm mt-1">Create actions linked to incidents, risks, or audits</p></div>
      ) : (
        <div className="glass-card overflow-hidden">
          <table className="w-full glass-table">
            ...
            <tbody>
              {actions.map((a) => (
                <tr key={a.id} className="border-b last:border-0 transition-colors hover:bg-white/[0.02]" style={{ borderColor: "var(--glass-border)" }}>
